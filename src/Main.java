...
private static class ExpressionParser {
    ...
    private double parse(String expression) {
        ...
        if (pos < expression.length()) throw new RuntimeException("Unexpected: " + (char) ch);  // Handle invalid characters
        return x;
    }
    ...
    private double parseTerm(String expression) {
        ...
        while (true) {
            ...
            } else if (eat('^', expression)) x = Math.pow(x, parseFactor(expression));  // Handle exponentiation (^) operator
    ...
    private double parseFactor(String expression) {
        ...
        if (eat('(', expression)) {
            ...
            while ((ch >= '0' && ch <= '9') || ch == '.') nextChar(expression); // Move through the digits of the number
